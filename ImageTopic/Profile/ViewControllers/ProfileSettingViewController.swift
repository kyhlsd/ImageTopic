...
final class ProfileSettingViewController: UIViewController {
    ...
    private func setupBindings() {
        viewModel.output.isEnableComplete.bind { [weak self] isEnabled in
            self?.completeButton.isEnabled = isEnabled
            self?.completeButton.backgroundColor = isEnabled ? .enabledButton : .disabledButton
        }
        viewModel.output.nicknameStatusText.lazyBind { [weak self] text in
            self?.nicknameStatusLabel.text = text
        }
        viewModel.output.isEnableNickname.lazyBind { [weak self] isEnabled in
            self?.nicknameStatusLabel.textColor = isEnabled ? .enabledButton : .systemRed
        }
        viewModel.output.mbti.lazyBind { [weak self] _ in
        ...
        viewModel.output.profile.lazyBind { [weak self] profile in
            ...
            self?.delegate?.setProfileImage(image: profile.image)
